...
import io.github.lunalobos.chess4kt.kingMatrix

internal fun yieldComputeVisible(): (square: Int, directionsIndexes: IntArray, directions: Array<IntArray>, friends: Long, enemies: Long) -> Long {
    ...
    return { square: Int, directionsIndexes: IntArray, directions: Array<IntArray>, friends: Long, enemies: Long ->
        ...
        for (index in directionsIndexes) {
            moves = (moves or getVisible(square, index, directions[index], friends, enemies))
        }
        moves
    }
}
...
internal inline fun yieldVisibleSquaresBishopAlternative(
    ...
    return { square: Int, friends: Long, enemies: Long ->
        computeVisible(
            square, bishopDirections, queenMegamatrix[square], friends, enemies
        )
    }
}
...
internal inline fun yieldVisibleSquaresRookAlternative(
    ...
    return { square: Int, friends: Long, enemies: Long ->
        computeVisible(
            square, rookDirections, queenMegamatrix[square], friends, enemies
        )
    }
}
...
internal class VisibleMetrics() {
    ...
    private val trailingZeros = IntArray(256)

    private val whitePawnCaptureMoves = whitePawnMatrix2
        .map { it.map { sq -> 1L shl sq }.reduceOrNull { acc, element -> acc or element } ?: 0L }.toLongArray()

    private val blackPawnCaptureMoves = blackPawnMatrix2
        .map { it.map { sq -> 1L shl sq }.reduceOrNull { acc, element -> acc or element } ?: 0L }.toLongArray()

    private val knightMovesMatrix = knightMatrix
        .map { it.map { sq -> 1L shl sq }.reduceOrNull { acc, element -> acc or element } ?: 0L }.toLongArray()

    private val kingMovesMatrix = kingMatrix
        .map { it.map { sq -> 1L shl sq }.reduceOrNull { acc, element -> acc or element } ?: 0L }.toLongArray()

    private val calculators = arrayOf(
        { _: Int, _: Long, _: Long -> throw IllegalArgumentException("piece must be between 1 and 12") },
        { sq: Int, f: Long, _: Long -> visibleSquaresWhitePawn(sq, f) },
        { sq: Int, f: Long, _: Long -> visibleSquaresKnight(sq, f) },
        { sq: Int, f: Long, e: Long -> visibleSquaresBishop(sq, f, e) },
    ...
    fun getVisible(square: Int, index: Int, direction: IntArray, friends: Long, enemies: Long): Long {
        // space transformation: board -> direction
        var fimage = 0
        var eimage = 0
        var counter = 0
    ...
    fun computeVisible(
        square: Int, directionsIndexes: IntArray, directions: Array<IntArray>, friends: Long, enemies: Long
    ): Long {
        var moves = 0L
        for (index in directionsIndexes) {
            moves = (moves or getVisible(square, index, directions[index], friends, enemies))
    ...
    fun visibleSquaresWhitePawn(square: Int, friends: Long): Long {
        return whitePawnCaptureMoves[square] and friends.inv()
    }

    fun visibleSquaresBlackPawn(square: Int, friends: Long): Long {
        return blackPawnCaptureMoves[square] and friends.inv()
    }

    fun visibleSquaresKnight(square: Int, friends: Long): Long {
        return knightMovesMatrix[square] and friends.inv()
    }

    fun visibleSquaresBishop(square: Int, friends: Long, enemies: Long): Long {
        return computeVisible(
            square, bishopDirections, queenMegamatrix[square], friends, enemies
        )
    }

    fun visibleSquaresRook(square: Int, friends: Long, enemies: Long): Long {
        return computeVisible(
            square, rookDirections, queenMegamatrix[square], friends, enemies
        )
    }

    fun visibleSquaresQueen(square: Int, friends: Long, enemies: Long): Long {
        val bishopVisible = visibleSquaresBishop(square, friends, enemies)
        val rookVisible = visibleSquaresRook(square, friends, enemies)
        return bishopVisible or rookVisible
    }

    fun visibleSquaresKing(square: Int, friends: Long): Long {
        return kingMovesMatrix[square] and friends.inv()
    }

    fun immediateThreats(bitboards: LongArray, friends: Long, enemies: Long): Long {
        var enemiesCopy = enemies
        var enemiesVisible = 0L
        while (enemiesCopy != 0L) {
            val bitboard: Long = enemiesCopy.takeLowestOneBit()
    ...
    fun visibleSquares(bits: LongArray, directionsIndexes: IntArray, square: Int, wm: Boolean): Long {
        val black = bits[6] or bits[7] or bits[8] or bits[9] or bits[10] or bits[11]
        val white = bits[0] or bits[1] or bits[2] or bits[3] or bits[4] or bits[5]
        val friends: Long
        val enemies: Long
