...
internal class Moves {
    ...
    fun moveFromOriginTarget(origin: Int, target: Int): Move {
        return moves[hash(origin, target)]
            ?: throw logger.fatal(MoveException("move for origin=$origin, target=$target not found"))
    }

    fun moveFromOriginTargetPromotion(origin: Int, target: Int, promotionPiece: Int): Move {
    ...
    fun moveFromOriginTargetObjects(origin: Square, target: Square): Move {
        return moves[hash(origin.ordinal, target.ordinal)]
            ?: throw logger.fatal(MoveException("move for origin=$origin, target=$target not found"))
    }

    fun moveFromOriginTargetPromotionObjects(origin: Square, target: Square, promotionPiece: Piece): Move {
        return moves[hash(origin.ordinal, target.ordinal, promotionPiece.ordinal)]
            ?: throw logger.fatal(
                MoveException(
    ...
    fun moveFromString(move: String): Move {
        return movesMap[move] ?: throw MoveException("$move is not a valid move string")
    }

    companion object {
        private val logger = getLogger("Moves")
    }
}